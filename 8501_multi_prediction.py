...
import numpy as np
import cv2
from predict_preprocess import process_image_multi, bbox_from_json_multi
...
# 2, load the data
# =============================================================================
bbox_file = 'examples/bbox2.json'
imgname = 'examples/test_img2.png'

centers, scales = bbox_from_json_multi(bbox_file)

imgs, norm_imgs = process_image_multi(imgname ,
                              bbox_file = bbox_file , openpose_file=None,
                              input_res=constants.IMG_RES)

# img_name = 'test_img2.png'
...
for i in range(len(imgs)-1):
    print(i)
    img, norm_img = imgs[i+1], norm_imgs[i+1]
    ...
    cv2.imwrite(outfile + '_shape%d.png'%i, 255 * img_shape[:,:,::-1])
